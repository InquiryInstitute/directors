...
async function applyPortraitUrls() {
  ...
  // First, ensure the portrait_url column exists
  console.log('📋 Ensuring portrait_url column exists...');
  const { error: alterError } = await supabase.rpc('exec_sql', {
    sql: `
      ALTER TABLE board_of_directors 
      ADD COLUMN IF NOT EXISTS portrait_url TEXT;
      
      CREATE INDEX IF NOT EXISTS idx_board_portrait_url 
      ON board_of_directors(portrait_url) 
  ...
  if (alterError) {
    ...
    // We'll proceed anyway - the column might already exist or we'll get a clearer error
  } else {
